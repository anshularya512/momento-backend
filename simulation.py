...
def get_future_events(user_id: str, db: Session):
    ...
    for inc in incomes:
        ...
        events.append({"date": next_date, "amount": inc.amount, "type": "credit", "source": "income"})

    for rec in recurrences:
        next_date = today + timedelta(days=rec.interval_days)
        events.append({"date": next_date, "amount": rec.amount, "type": "debit", "source": rec.merchant})
    return events
...
def simulate_30_days(user_id: str, db: Session):
    ...
    # Day 0: Current State
    simulation.append({"day": 0, "date": str(today), "balance": round(balance, 2)})
        ...
        for e in events:
            if e["date"] == date:
                if e["type"] == "credit":
                    ...
                    balance -= e["amount"]
        simulation.append({"day": day, "date": str(date), "balance": round(balance, 2)})
    return simulation

def forecast_cash_window(balance, avg_daily, volatility, buffer=500.0):
    if balance < buffer:
        return {"range_days": [0, 2], "confidence": 0.99, "state": "warning"}
    ...
    return {
        ...
        "confidence": 0.75,
        "state": "warning" if lower < 10 else "safe"
    }